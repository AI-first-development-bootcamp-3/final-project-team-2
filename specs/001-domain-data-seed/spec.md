...
A teammate needs to reset their local environment. They can re-run the seed safely so the demo organization returns to a known good state without leaving the database in a broken or duplicated-unusable state.

**Why this priority**: Local reset is important for day-to-day development, but secondary to first-time schema + seed succeeding.

**Independent Test**: Run the seed twice in a row (or run seed after an intentional local reset) and verify the demo org remains usable with the required minimum counts and relationships.

**Acceptance Scenarios**:

1. **Given** a database that already contains demo seed data, **When** the seed is run again, **Then** the operation completes safely and leaves a usable demo organization matching the minimum seed requirements.
2. **Given** a teammate wants a clean local demo, **When** they re-apply seed after reset, **Then** they can continue frontend development against the known demo org without manual data cleanup.
...
- What happens if seed fails halfway through (partial demo org)—can a subsequent run recover to a usable state?
...
- **FR-008**: Seed MUST be safely re-runnable for local reset so teammates can restore a usable demo organization without manual repair.
...
- **SC-005**: Re-running seed for local reset leaves a usable demo organization matching SC-002 without requiring manual data repair.
...
- Seed re-runnability means local development can reset to a known good demo state safely (idempotent seed or equivalent safe reset behavior).
