...
import { REFRESH_COOKIE, DAY_MS } from './auth.constants';
...
describe('remember-me refresh durations', () => {
  ...
  it('unchecked: cookie Max-Age and token claim are both ~1 day', async () => {
    const cookie = await loginRefreshCookie(false);
    expect(cookieMaxAgeSeconds(cookie)).toBe(DAY_MS / 1000);
    expect(refreshTtlSeconds(cookie)).toBe(DAY_MS / 1000);
  });
  ...
  it('checked: cookie Max-Age and token claim are both ~30 days', async () => {
    const cookie = await loginRefreshCookie(true);
    expect(cookieMaxAgeSeconds(cookie)).toBe((30 * DAY_MS) / 1000);
    expect(refreshTtlSeconds(cookie)).toBe((30 * DAY_MS) / 1000);
  });
