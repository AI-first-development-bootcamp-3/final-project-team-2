 ...
 * belongs to the Month Close epic (§7.2). Auth is the global APP_GUARD pair
 * (JwtGuard → RolesGuard), same as /me and /time-entries.
 */
@ApiTags('months')
@ApiBearerAuth()
@Controller('months')
...
export class MonthsController {
  ...
  @ApiOperation({
    ...
    description:
      'Entries, absences, and lock status in a single response. Absences are the ' +
      "caller's own, overlapping the month, both bounds inclusive.",
  })
