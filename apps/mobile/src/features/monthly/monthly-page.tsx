 ...
 * back empty (spec §3). The month is refetched on every mount, so returning
 * from the edit form (KAN-82) always shows the post-edit statuses.
 */
export function MonthlyPage() {
  ...
  return (
    <div dir="rtl" className="mx-auto flex max-w-[393px] flex-col gap-4 p-4 font-sans">
      ...
      {load.status === 'ready' && (
        <>
          {load.data.lock.isLocked && (
            <div className="flex flex-col gap-2">
              <LockStatusIndicator
                year={year}
                month={month}
                isLocked={load.data.lock.isLocked}
                lockedAt={load.data.lock.lockedAt}
              />
          ...
          {selectedDay !== null && (
            <DayDetail
              date={selectedDay}
              entries={load.data.entries}
              locked={load.data.lock.isLocked}
            />
          )}
        </>
      )}
