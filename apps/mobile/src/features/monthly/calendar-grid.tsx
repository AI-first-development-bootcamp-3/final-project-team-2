...
/**
 * A reportless Friday or Saturday is not a day with missing hours, so it must
 * not read like an EMPTY working day; a weekend that *was* worked or covered
 * by an absence keeps its status color.
 */
const WEEKEND_EMPTY_CLASSES = 'bg-transparent text-neutral-300';
...
/** Friday (5) and Saturday (6) — the Israeli weekend (spec §1). */
...
export function CalendarGrid({ year, month, entries, absences, onSelectDay }: CalendarGridProps) {
  ...
  const days = Array.from({ length: total }, (_, index) => {
    ...
    return { date, day: index + 1, status, weekend: isWeekend(date) };
  });
  ...
  return (
    <div role="grid" dir="rtl" aria-label="לוח חודשי" className="grid grid-cols-7 gap-1">
      ...
      {days.map(({ date, day, status, weekend }) => (
        <button
          key={date}
          type="button"
          role="gridcell"
          ...
          className={`aspect-square rounded-lg text-sm font-medium ${
            weekend && status === 'empty' ? WEEKEND_EMPTY_CLASSES : STATUS_CLASSES[status]
          }`}
        >
          {day}
        </button>
