...
export function ClientCreateForm({ open, onClose, onCreated }: ClientCreateFormProps) {
  ...
  async function handleSubmit() {
    ...
    const parsed = CreateClientBodySchema.safeParse({ name, contactInfo: contactInfo || undefined });
    if (!parsed.success) {
    ...
    >
      ...
      <label className="flex flex-col text-sm">
        ...
        <textarea
          ...
          rows={2}
        />
        {fieldErrors.contactInfo ? <p role="alert" className="text-sm text-red-600">{fieldErrors.contactInfo}</p> : null}
      </label>
